...
#' @rdname  Utils
#' @export
get.varimp <- function(object, alpha = 0.05){
if(!object$varimp) stop("no variable importance available in model") 

cls <- c("GLM", "GBM", "RF")
tab <- do.call(rbind.data.frame, lapply(object$CV.res, function(x) {   ## each CV 
models <- names(x) 
do.call(rbind.data.frame, lapply(models,  function(y){  ## outcome 
...
cbind.data.frame(Outcome = y, GLM.vars = GLM.vars, GLM.imp = GLM.imp, GBM.vars = GBM.vars,
...
tab <- cbind.na(GLM, GBM[, ,drop=F], RF[, ,drop=F])
rownames(tab) <- NULL
tab
})
